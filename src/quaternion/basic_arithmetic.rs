...
impl<T: TQ<T>> std::ops::Neg for Quaternion<T> {
    type Output = Self;
    fn neg(self) -> Self { Quaternion::new(-self.r, -self.i, -self.j, -self.k) }
}

impl<T: TQ<T>> std::ops::Add for Quaternion<T> {
    type Output = Self;
    fn add(self, other: Self) -> Self { Quaternion::new(self.r + other.r, self.i + other.i, self.j + other.j, self.k + other.k) }
}

impl<T: TQ<T>> std::ops::Sub for Quaternion<T> {
    type Output = Self;
    fn sub(self, other: Self) -> Self { Quaternion::new(self.r - other.r, self.i - other.i, self.j - other.j, self.k - other.k) }
}

impl<T: TQ<T>> std::ops::Mul<T> for Quaternion<T> {
    type Output = Self;
    fn mul(self, rhs: T) -> Self { Quaternion::new(self.r * rhs, self.i * rhs, self.j * rhs, self.k * rhs) }
}
...
impl std::ops::Mul<Quaternion<f64>> for f64 {
    ...
    fn mul(self, rhs: Quaternion<f64>) -> Quaternion<f64> { rhs * self }
}

impl<T: TQ<T>> std::ops::Mul<Quaternion<T>> for Quaternion<T> {
